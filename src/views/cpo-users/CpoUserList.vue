...
import { CpoUserService } from '@/service/CpoUserService';
...
async function loadUsers() {
    loading.value = true;
    try {
        users.value = await CpoUserService.getCpoUsers();
    } catch (error) {
        console.error('Error loading CPO users:', error);
    } finally {
        loading.value = false;
    }
}
...
async function saveUser() {
    if (editingId.value) {
        await CpoUserService.updateCpoUser(editingId.value, formData.value);
    } else {
        await CpoUserService.createCpoUser(formData.value);
    }
...
async function deleteUser(id) {
    if (confirm('Kullanıcıyı silmek istiyor musunuz?')) {
        await CpoUserService.deleteCpoUser(id);
        await loadUsers();
    }
}
</script>
        ...
        <DataTable :value="users" :loading="loading" paginator :rows="20" dataKey="id" filterDisplay="menu">
            <template #header>
                <div class="flex justify-between items-center gap-4">
                    <span class="text-xl font-semibold">CPO Kullanıcıları</span>
            ...
            <Column field="tenantName" header="Kiracı" sortable style="width: 12%" />
            <Column field="status" header="Durum" style="width: 10%">
                <template #body="{ data }">
                    <Tag :value="data.status" :severity="getStatusSeverity(data.status)" />
                </template>
            </Column>
            <Column header="İşlemler" style="width: 14%">
                <template #body="{ data }">
    ...
    <Dialog v-model:visible="showDialog" :header="editingId ? 'Kullanıcıyı Düzenle' : 'Yeni Kullanıcı'" modal>
        <div class="grid grid-cols-1 gap-4 mb-4">
            ...
            <div>
                ...
                <Dropdown v-model="formData.role" :options="['Manager', 'Operator', 'Supervisor']" class="w-full" />
            </div>
            <div>
